...
- Gate private pages via `PROTECTED_ROUTES` in `@src/middleware.ts` (currently `/dashboard`, `/runs/new`, `/admin`, `/runs/history`). The `/admin` prefix includes `/admin/users/{id}`; non-admins still 404.
