...
- Gate private pages via `PROTECTED_ROUTES` in `@src/middleware.ts` (currently `/dashboard`, `/runs/new`, `/admin`, `/runs/history`, `/profile`, plus `/runs/{id}/edit`). `/players/{id}` is public — do not add it to `PROTECTED_ROUTES`. Friend mutations are `POST /api/friends/*` (`request`, `accept`, `decline`, `cancel`, `unfriend`); the pending inbox is on `/profile`, not the public player page. The `/admin` prefix includes `/admin/users/{id}`; non-admins still 404. Do not prefix-protect `/runs` — the public list and `/runs/{id}` stay open.
