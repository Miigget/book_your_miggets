...
- Gate private pages via `PROTECTED_ROUTES` in `@src/middleware.ts` (currently `/dashboard`, `/runs/new`, `/admin`, `/runs/history`, `/profile`, plus `/runs/{id}/edit`). `/players/{id}` is public — do not add it to `PROTECTED_ROUTES`. `/auth/confirm` is public (PKCE `code` / `token_hash` from Auth emails; also handled in middleware if the link lands on Site URL). `/auth/verified` is public — the post-confirm success screen; do not add it to `PROTECTED_ROUTES`. Friend mutations are `POST /api/friends/*` (`request`, `accept`, `decline`, `cancel`, `unfriend`); the pending inbox is on `/profile`, not the public player page. The `/admin` prefix includes `/admin/users/{id}`; non-admins still 404. S-16 mutations (nickname, KoG points, points verified, nickname-request) live on `/admin/users/{id}` via `POST /api/admin/users/{id}/*`. Do not prefix-protect `/runs` — the public list and `/runs/{id}` stay open.
