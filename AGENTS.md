...
- Gate private pages via `PROTECTED_ROUTES` in `@src/middleware.ts` (currently `/dashboard`, `/runs/new`, `/admin`, `/runs/history`, `/profile`). `/players/{id}` is public. The `/admin` prefix includes `/admin/users/{id}`; non-admins still 404.
