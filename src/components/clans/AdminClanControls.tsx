...
import { ImagePlus, Save, Tag, Trash2, Users } from "lucide-react";
...
export default function AdminClanControls({ clanId, name: initialName, tag: initialTag, serverError }: Props) {
  ...
  async function onDelete(event: React.SubmitEvent<HTMLFormElement>) {
    event.preventDefault();
    const ok = window.confirm("Delete this clan permanently? Members will be removed from the roster.");
    if (!ok) return;
    setBusy(true);
    setDeleteError(null);
    try {
      const { response, data } = await fetchFormJson(event.currentTarget);
      if (response.status === 401 && data.signIn) {
        window.location.assign(data.signIn);
        return;
      }
      if (!response.ok) {
        setDeleteError(data.error ?? "Could not delete this clan");
        return;
      }
      window.location.assign(data.redirect ?? "/clans");
    } catch {
      setDeleteError("Could not delete this clan");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div>
          <label htmlFor="admin-clan-picture" className="mb-1 block text-sm text-blue-100/80">
            Picture <span className="font-normal text-blue-100/40">(optional — leave empty to keep)</span>
          </label>
      ...
      <form
        method="POST"
        action={`/api/admin/clans/${clanId}/delete`}
        onSubmit={(event) => {
          event.preventDefault();
          void onDelete(event);
        }}
