...
import { closeMapPoll, PollError } from "@/lib/services/polls";
...
export const POST: APIRoute = async (context) => {
  ...
  try {
    await closeMapPoll(supabase, runId);
  } catch (err) {
    if (err instanceof PollError) {
      return fail(err.message);
    }
    console.error("closeMapPoll failed", err);
    return fail("Could not close this poll");
  }
