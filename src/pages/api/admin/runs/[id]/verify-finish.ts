...
import { verifyClanRunFinish, isUuid, RunError } from "@/lib/services/runs";
...
export const POST: APIRoute = async (context) => {
  ...
  try {
    await verifyClanRunFinish(supabase, runId);
  } catch (err) {
    if (err instanceof RunError) {
      return fail(err.message);
    }
    console.error("verifyClanRunFinish failed", err);
    return fail("Could not verify this clan run");
  }
