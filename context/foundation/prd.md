...
- Past runs do not clutter the active list — unextended runs leave 1 hour after start; the organizer or an admin can archive earlier (optional organizer extend of at most 6 hours, then derived exit); archived, not deleted. An organizer may have at most 5 audience-active runs.
...
- **Then** the organizer can accept or deny the applicant, confirmed players appear on the participant list, and the run remains on the active list for 1 hour after start unless the organizer or an admin archives it earlier (or an optional organizer extend of at most 6 hours elapses). Confirmed participants can reopen the archive. An organizer may have at most 5 audience-active runs.
