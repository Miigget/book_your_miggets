...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: context type
      decision: greenfield — new web app, no existing application code in repo
    - topic: pain category
      decision: missing capability + coordination overhead — no built-in team finder/run scheduler; high effort to find players at similar skill level
    - topic: insight
      decision: nobody in the KoG community built it yet
    - topic: primary persona scope
      decision: run organizer — experienced KoG player who creates runs
    - topic: auth strategy
      decision: MVP email+password; Discord OAuth deferred to v2
    - topic: role model
      decision: MVP guest browse + member (create/join) + admin moderation
    - topic: scale vs approval bottleneck
      decision: organizer-selectable join mode — approval-required or auto-join per run
    - topic: archive UX
      decision: confirmed participants see their archived runs; admin views any player's archive via profile
    - topic: archive retention
      decision: retain indefinitely in MVP; deletion or cold storage deferred until scale demands it
  frs_drafted: 15
...
The King of Gores (KoG) community in TeeWorlds is relatively small, and the game client offers no tools for organizing shared runs. Finding the right players, agreeing on a time, and filling a team for harder maps happens mainly through in-game chat, server-hopping, Discord, and private messages — which is inconvenient, time-consuming, and often fails.
...
**Insight:** The niche is real and the pain is acute, but nobody in the community has shipped a dedicated tool yet — the status quo (Discord + in-game coordination) persists by default, not by choice.
...
- Discord OAuth + KoG Discord server integration planned for v2 (player verification).
...
- FR-012: Admin can mark a user as verified (`is_verified` flag). Priority: must-have
  > Socrates: No counter-argument; minimal trust signal for v1 until Discord verification ships.
...
- **Avoid: archive deletion or cold storage** — archived runs are retained indefinitely in MVP; retention policy can be revisited if popularity drives storage pressure.

## Resolved decisions (formerly open questions)

- **Scale vs approval bottleneck:** Organizer chooses join mode per run — approval-required for skill gating, auto-join to speed team formation at scale. Both ship in MVP (FR-003, FR-004, FR-014).
- **Archive UX:** Confirmed participants can view archived runs they took part in. Admin can access any player's archived run history via that player's profile.
- **Archive retention:** No deletion or cold storage in MVP — archives kept indefinitely. Revisit only if the site grows enough to create storage pressure.

## Quality cross-check

All elements present — no gaps at handoff.

- Access Control: documented (guest / member / admin + deferred roles)
- Business Logic: one-sentence organizer-accept gate with supporting detail
- Timeline-cost: 3-week after-hours MVP acknowledged
- Non-Goals: three explicit scope avoids captured
