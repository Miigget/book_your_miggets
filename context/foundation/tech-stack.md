...
tag `v*` via `/gh-release` so GitHub Release notes exist at ship time. Timed run archival (FR-013) for
MVP uses derived status at read time (plus matching RLS active-window SELECT policies); add a
Cloudflare Cron Trigger only if a later slice requires stamped `archived_at`. Scaffolding confidence is first-class: expect
